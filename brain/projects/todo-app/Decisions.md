...
### 2026-07-22 — Inline natural-language parsing when typing a task (Todoist-style)
**Why:** Typing "завтра о 16:00" now recognises date/time/priority locally (new `lib/nlp.ts`, synchronous, bilingual UK+EN) — `/api/parse` stays voice-only (a per-keystroke LLM round-trip is wrong). Tokens highlight in the brand colour via a transparent `<input>` + mirror layer (`HighlightedInput`), auto-fill the chips, and are stripped from the saved title. Provenance flags keep parsing from ever clobbering a chip the user set by hand. v1 scope = date+time+priority only (repeat/deadline/per-token-dismiss deferred). Spec: `brain/projects/todo-app/plans/2026-07-22-inline-nlp-design.md`.
